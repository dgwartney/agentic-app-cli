...
from agxr.exceptions import AgenticAPIError
from agxr.logging_config import setup_logging, get_logger
...
Examples:
  # Profile management
  agxr profile add                    # Add profile interactively
  agxr profile add --name prod --api-key kg-... --app-id aa-...
  agxr profile list                   # List all profiles
  agxr profile list --show-keys       # Show full API keys
  agxr profile set-default prod       # Set default profile
  agxr profile delete staging         # Delete a profile

  # Execute with auto-generated session ID
  agxr execute --query "Hello"
  agxr execute --query "What is the weather?"

  # Execute with custom session ID
  agxr execute --query "Test" --session-id custom-session-001

  # Execute with profiles
  agxr --profile prod execute --query "Hello"

  # Execute with options
  agxr execute --query "Explain AI" --stream tokens --debug --debug-mode thoughts

  # Use different environment
  agxr execute --env-name stage --query "Test"

  # Interactive chat
  agxr chat                           # Start chat with auto-generated session
  agxr chat --session-id my-session   # Start chat with specific session
  agxr chat --stream tokens --debug   # Chat with streaming and debug
  ...
  KOREAI_TIMEOUT       Request timeout in seconds (default: 30)
    ...
    def _handle_profile_list(self, args: argparse.Namespace, manager) -> int:
        ...
        if not profiles:
            ...
            print("\nTo add a profile, run: agxr profile add")
            return 0
    ...
    def _handle_profile_delete(self, args: argparse.Namespace, manager) -> int:
        ...
        if default_profile is None and manager.list_profiles():
            ...
            print("  agxr profile set-default <name>")
...
def main() -> NoReturn:
    ...
    This function is called when the agxr command is executed.
    """
    cli = CLI()
    sys.exit(cli.run())
